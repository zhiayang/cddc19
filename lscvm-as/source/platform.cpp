...
namespace platform
{
	...
	filehandle_t openFile(const char* name, int mode, int flags)
	{
			...
			if(hd == INVALID_HANDLE_VALUE)
				...
				// fprintf(stderr, "Failed to open file '%s' (error code %d)", name, GetLastError()), exit(-1);
	...
	std::string getFullPath(const std::string& partial)
	{
		#if PLATFORM_IS_WINDOWS
		{
			...
			if(hd == INVALID_HANDLE_VALUE)
				return "";
				// fprintf(stderr, "Failed to open file '%s' (error code %d)", partial, GetLastError()), exit(-1);
	...
	{
		#if PLATFORM_IS_WINDOWS || defined(__MINGW32__)
		{
