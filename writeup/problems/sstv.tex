...
\section{Super Strong TeleVision}
	...
	\subsection{Solution}
		...
		and making QSSTV record from the \ttt{null sink}:
		
		\begin{figure}[!htbp]
			\centering
			\includegraphics[width=130mm]{figures/sstv/settings.png} \vspace{5mm}
			\caption{QSSTV -- Set it to retrieve inputs from \itl{PulseAudio} instead of the default \itl{JACK}}
		\end{figure}
		...
		and use VLC to playback the \ttt{.wav} file to the \ttt{null-sink}:
		...
		Thankfully most of the decoding is handled automatically by \itl{QSSTV} and do not require too much manual intervention. 
	% end subsection
